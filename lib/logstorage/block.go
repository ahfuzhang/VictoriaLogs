...
func (c *column) canStoreInConstColumn() bool {
	...
	// Before comparing string contents, compare string lengths first.
...
func (b *block) assertValid() {
	...
	// Quickly check the entire timestamp array, faster than comparing each one
...
func areTimestampsSorted(ts []int64) bool {
	...
	for i := 1; i < align4+1; i += 8 {
		arr := (*[9]int64)(unsafe.Add(ptr, (i-1)*8)) // compare 8 int64s at once
		sign := ((arr[1] - arr[0]) >> 63) |
			((arr[2] - arr[1]) >> 63) |
			((arr[3] - arr[1]) >> 63) |
			((arr[4] - arr[3]) >> 63) |
